...
本アプリにおける01仕様は、`301` / `501` / `701` のようなゲーム全体進行ではなく、**終盤のアウト（チェックアウト）局面**を対象とします。  
学習モードとタイムアタックモードは、いずれも「指定スコアに対して3投以内のチェックアウトを入力し、正誤判定する」問題モードとして動作します。

### 基本ルール

- 1ターンの入力は最大3投
- 合計点が目標スコアと一致し、かつ最終投がルール条件を満たすと正解
- 最終投の有効条件は `OutRule` で切り替え

| ルール | 最終投の有効条件 |
|--------|------------------|
| open | 制約なし（すべて有効） |
| double | `double` または `bullseye` |
| master | `double` / `triple` / `bullseye` |

### 出題仕様

- 出題対象スコアは `21〜180`
- `questionGenerator.ts` で「そのルールで実際に上がれる点数」のみをプール化
- 直近10問は再出題しない

### 判定仕様

- プレイヤー入力の3投以内の合計が問題スコアと一致していることを確認
- 一致時は最終投に `isValidFinish(dart, outRule)` を適用
- 不一致または最終投ルール違反は不正解
- 正誤判定後、`generateCheckouts(score, outRule)` で模範解答を提示

### モード別の扱い

- 学習モード: 1問ごとに正誤と模範解答を表示し、正答率を更新
- タイムアタック: 同判定ロジックで10問連続実行し、所要時間と正答数を記録
